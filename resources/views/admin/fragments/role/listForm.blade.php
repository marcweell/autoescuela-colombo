<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.admin.user.user_group.add.index') }}" data-id="-1" class="btn btn-primary mb-2 _link_"><i
                        class="mdi mdi-plus-circle me-2"></i> {{ __('Adicionar Grupo de Usuarios') }}</a>
            </div>
            <div class="col-sm-7">
        ...
        <div class="table-responsive--">
            <table class="table table_ table-sm table-smtable-centered w-100 dt-responsive nowrap" id="products-datatable">
                ...
                <tbody>
                    @for ($i = 0, $n = 1; $i < count($user_group ?? []), ($item = @$user_group[$i]); $i++, $n++)
                        <tr>
                            <td> {{ $n }} </td>
                            <td> {{ $item->name }} </td>
                            <td> {{ $item->code }}
                            ...
                            <td class="table-action">
                                <a  data-href="{{ route('web.admin.user.user_group.group_permission.index') }}"
                                data-id='{{ $item->id }}' class="btn btn-primary _link_" class="btn btn-primary _link_"> <i class="fa fa-shield-alt"></i></a>
                                <a data-href="{{ route('web.admin.user.user_group.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-primary _link_"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.admin.user.user_group.remove.do') }}"
