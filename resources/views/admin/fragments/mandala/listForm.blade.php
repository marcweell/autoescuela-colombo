@php
    $cols = ['counter','code', 'plan_name', 'amount', 'created_at', 'action'];
    $cols = json_encode($cols);
    $apiUrl = route('web.admin.mandala.index');
    $dataKey = 'mandala';
@endphp

<div class="card">
    <div class="card-header">
        <div class="card-title">
            <h5>Matrizes<h5>
        </div>
    </div>
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.admin.mandala.add.index') }}" data-id="-1"
                    class="btn btn-secondary  mb-2 l14k"><i class="fa fa-plus-circle me-2"></i>
                    {{ __('Adicionar Matriz') }}</a>
            </div>
            <div class="col-sm-7">
                <div class="text-sm-end">

                </div>
            </div><!-- end col-->
        </div>

        <div class="table-responsive">
            <table class="table table_sourced table-centered w-100 dt-responsive" data-method="POST" data-key="{{ $dataKey }}" data-url="{{ $apiUrl }}" data-cols="{{ $cols }}">
                <thead class="">
                    <tr>
                        <th style="width: 20px;">
                            #
                        </th>
                        <th>{{ __('Codigo') }}</th>
                        <th>{{ __('Plano') }}</th>
                        <th>{{ __('Custo') }}</th>
                        <th>{{ __('Data/Hora de Registo') }}</th>
                        <th style="width: 85px;"><i class='fa fa-cog'></i></th>
                    </tr>
                </thead>
                <tbody>
                    @for ($i = 0, $n = 1; $i < count([]), ($item = @$mandala[$i]); $i++, $n++)
                        <tr>
                            <td> {{ $n }} </td>
                            <td> {{ $item->code }}
                            </td>
                            <td> {{ $item->plan_name }}
                            </td>
                            <td>{{ $item->currency_symbol . format_number($item->price ?? 0) }} </td>
                            <td> {{ tools()->date_convert($item->created_at) }} </td>
                            <td class="table-action">
                                <a data-href="{{ route('web.admin.mandala.participant.manage.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  l14k"> <i
                                        class="fa fa-cog"></i> Gerenciar</a>
                                <a data-href="{{ route('web.admin.mandala.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  l14k"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.admin.mandala.remove.do') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  l14k prompt"
                                    data-title="Remover Matriz"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endfor
                </tbody>
            </table>
        </div>
    </div> <!-- end card-body-->
</div> <!-- end card-->
