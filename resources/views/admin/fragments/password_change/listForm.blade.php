<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.admin.geo.password_change.add.index') }}" data-id="-1" class="btn btn-secondary  mb-2 l14k"><i
        ...
        <div class="table-responsive">
            <table class="table table_ table-centered w-100 dt-responsive">
                ...
                <tbody>
                    @for ($i = 0, $n = 1; $i < count($password_change ?? []), ($item = @$password_change[$i]); $i++, $n++)
                        <tr>
                            <td> {{ $n }} </td>
                            <td> {{ $item->name }} </td>
                            <td> {{ $item->code }}
                            ...
                            <td class="table-action">
                                ...
                                <a data-href="{{ route('web.admin.geo.password_change.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  l14k"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.admin.geo.password_change.remove.do') }}"
