<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.admin.settings.course_category.add.index') }}" data-id="-1" class="btn btn-primary mb-2 _link_"><i
                        class="mdi mdi-plus-circle me-2"></i> {{ __('Adicionar Categoria de Curso') }}</a>
            </div>
            <div class="col-sm-7">
        ...
        <div class="table-responsive--">
            <table class="table table_ table-sm table-smtable-centered w-100 dt-responsive nowrap" id="products-datatable">
                ...
                <tbody>
                    @for ($i = 0, $n = 1; $i < count($course_category ?? []), ($item = @$course_category[$i]); $i++, $n++)
                        <tr>
                            <td> {{ $n }} </td>
                            <td> {{ $item->name }} </td>
                            <td> {{ $item->code }}
                            ...
                            <td class="table-action">
                                ...
                                <a data-href="{{ route('web.admin.settings.course_category.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-primary _link_"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.admin.settings.course_category.remove.do') }}"
