...
<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.app.question.category.add.index') }}" data-id="-1"
                    ...
                    @for ($i = 0, $n = 1; $i < count($question_category ?? []), ($item = @$question_category[$i]); $i++, $n++)
                        <tr>
                            ...
                            <td class="table-action">
                                <a data-href="{{ route('web.app.question.category.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary btn-sm _link_"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.app.question.category.remove.do') }}" data-id='{{ $item->id }}'
                                    class="btn btn-secondary btn-sm _link_ prompt" data-title="Eliminar question_category"><i
