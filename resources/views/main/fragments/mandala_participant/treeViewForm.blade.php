...
<div class="card">
    <div class="card-header">
        ...
        <button data-id="{{ $mandala->id }}" class="btn btn-lg btn-dark m-3 l14k"
            data-href="{{ route('web.app.mandala.participant.manage.index') }}"><i class="fa fa-reply"></i>
            {{ __("VOLTAR") }}</button>
    </div>
    <div class="card-body">
        ...
        <div class="row d-none d-lg-flex bhr">
            ...
            <div class="col-12 col-lg b_info text-uppercase">
                {{ __("doadores") }}
            </div>
            <div class="col-12 col-lg  b_info text-uppercase">
                {{ __("construtores") }}
                    ...
                    @foreach ($tree1 as $pattern)
                        @foreach ($pattern->children as $item)
                            <button class="btn offset-2 offset-lg-0 col-3 club-info col-lg-12 min-h  xtool"
                                ...
                                <img data-src="{{ tools()->photo($mandala->plan_icon) }}" class="w-100 shadow lazy"  style="{{ in_array($item->id,$paids)?"border-color: #4CAF50!important;":(in_array($item->id,$unlocks)?"border-color: #f38f38!important;":"") }}">
                                <label
                ...
                <div class="row ">
                    @foreach ($tree1 as $item)
                        <button class="btn col-3 club-info mentor_ col-lg-12 min-h offset-4-5 offset-lg-0  xtool"
                            title="{{ user_data($item->user_id, $item->type, $item->id, $paids) }}">
                            <img data-src="{{ tools()->photo($mandala->plan_icon) }}" class="w-100 shadow lazy"  style="border-color: #4caf50!important;">
                            <label
                                class="{{ $item->id == null ? 'force-white' : 'text-dark' }} text-center">{{ $item->user_code }}</label>
                        </button>
                    @endforeach
            ...
            <div class="col-12 col-lg">
                <div class="row "  >
                    <button class="btn col-3 club-info mentor_ col-lg-12 min-h offset-4-5 offset-lg-0 xtool"
                        ...
                        <img data-src="{{ tools()->photo($mandala->plan_icon) }}" class="w-100 shadow lazy"  style="border-color: #4caf50!important;width: 105%;max-width: 130px!important;position: relative;top: -14px;">
