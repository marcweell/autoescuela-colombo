<div class="card">
    ...
    <div class="card-body">
        <h4 class="header-title">{{ __('Desbloquear Doacao') }}</h4>
        <form action="{{ route('web.app.mandala.unlock.add.do') }}" class="form_ parent-load-- row" method="post">
            <div class="read-content">
                <div class="media pt-3 d-sm-flex d-block justify-content-between row">
                    ...
                    <div class="col-md-6 mb-3">
                        <h4 class="d-block">{{ __("Informacoes de Indicador") }}</h4>
                        <div class="d-flex">
                    ...
                    <div class="media-body me-2">
                        ...
                        <ul>
                            ...
                            <li>
                                <a href="https://wa.me/{{$item->profile_id}}" class="btn btn-success"><i class="fa fa-paper-plane"></i> Mandar Mensagem</a>
                            </li>
                        ...
                        <div class="media mb-2">
                            <h5 class="my-1 text-primary"><i class="fa fa-coins"></i>
                                {{ $mandala->currency_symbol . format_number($mandala->price * 0.25) }} ({{ convertCoin($mandala->currency_code,"USD",$mandala->price * 0.75,"USD") }})</h5>
                        </div>
                        ...
                        <select name="payment_method_id" id="user_id" class="form-control">
                            @foreach ($payment_method as $item)
                                <option value="{{ $item->id }}"> {{ $item->name }}{{ !empty($item->account_number_type) and strtolower($item->name) == "pix" ? " ({$item->account_number_type})":""  }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
