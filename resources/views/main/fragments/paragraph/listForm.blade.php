...
<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.admin.paragraph.add.index') }}" data-id="-1"
        ...
        <div class="table-responsive">
            <table class="table table_ table-centered w-100 dt-responsive">
                ...
                <tbody>
                    @for ($i = 0, $n = 1; $i < count($paragraph ?? []), ($item = @$paragraph[$i]); $i++, $n++)
                        <tr>
                            <td> {{ $n }} </td>
                            <td>
                                {{ $item->name }}
                            ...
                            <td class="table-action">
                                <a data-href="{{ route('web.admin.paragraph.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary btn-sm _link_"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.admin.paragraph.remove.do') }}" data-id='{{ $item->id }}'
                                    class="btn btn-secondary btn-sm _link_ prompt" data-title="Eliminar paragraph"><i
