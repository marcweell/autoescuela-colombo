<div class="card">
    ...
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-sm-5">
                <a data-href="{{ route('web.app.settings.notification.add.index') }}" data-id="-1" class="btn btn-secondary  mb-2 _link_"><i
                    ...
                    @for ($i = 0, $n = 1; $i < count($notification ?? []), ($item = @$notification[$i]); $i++, $n++)
                        <tr>
                            ...
                            <td class="table-action">
                                ...
                                <a data-href="{{ route('web.app.settings.notification.update.index') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  _link_"><i
                                        class="fa fa-edit"></i></a>
                                <a data-href="{{ route('web.app.settings.notification.remove.do') }}"
                                    data-id='{{ $item->id }}' class="btn btn-secondary  _link_ prompt"
                                    data-title="Eliminar"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endfor
                </tbody>
