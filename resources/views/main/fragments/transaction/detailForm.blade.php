        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <ul>
                    ...
                    <li class=""> Montante:
                        {{ $transaction->currency_symbol . format_number($transaction->amount ?? 0) }} </li>
                    ...
                    <li class=""> Data de Pagamento:
                        {{ tools()->date_convert($transaction->payment_date, 'd/m/Y') }} </li>
                        @if (!empty($transaction->debt_user_id) and $transaction->type == 'credit')
                            <li class=""> Pago Por: {{ $transaction->debt_user_full_name }} {{ empty($transaction->debt_user_phone)?"":" - ".$transaction->debt_user_phone }} </li>
                        @endif
                        @if (!empty($transaction->credit_user_id) and $transaction->type == 'debt')
                            <li class=""> Beneficiario: {{ $transaction->credit_user_full_name }} {{ empty($transaction->credit_user_phone)?"":" - ".$transaction->credit_user_phone }} </li>
                        @endif
