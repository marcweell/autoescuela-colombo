...
class LanguageController extends Controller
{
    ...
    {
        try {
            $language = $this->languageServiceQuery->deleted(false)->orderDesc()->findAll();
            $view = view('admin.fragments.language.listForm', [
                'language' => $language
            ])->render();
    ...
    {
        try {
            $view = view('admin.fragments.language.addForm', [
            ])->render();
    ...
    public function updateIndex(Request $request)
    {
        ...
        try {
            $language = $this->languageServiceQuery->deleted(false)->orderDesc()->findById($request->get('id'));
            $view = view('admin.fragments.language.editForm', [
                'language'=>$language
            ])->render();
            return (new WebApi())->setSuccess()->print($view,'modal')->get();
        } catch (\Exception $e) {
