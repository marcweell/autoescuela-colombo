...
class Course_categoryController extends Controller
{
    ...
    {
        try {
            $course_category = $this->course_categoryServiceQuery->findAll();
            $view = view('user.fragments.course_category.listForm', [
    ...
    {
        try {
            $view = view('user.fragments.course_category.addForm', [
            ])->render();
    ...
    public function updateIndex(Request $request)
    {
        ...
        try {
            $course_category = $this->course_categoryServiceQuery->findById($request->get('id'));
            $view = view('user.fragments.course_category.editForm', [
                'course_category'=>$course_category
            ])->render();
            return (new WebApi())->setSuccess()->print($view,'modal')->get();
        } catch (\Exception $e) {
