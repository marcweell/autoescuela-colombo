...
class System_settingsServiceImpl implements ISystem_settingsService
{
    ...
    public function add($data)
    {
        ...
        $system_settings = DB::table($this->table)->where('name', $data->name)->first();

        if (!empty($system_settings->id)) {
            throw new \Exception(__('Nome invalido'), 400);
        }

        
        
        $system_settings = DB::table($this->table)->where('code', $data->code)->first();
    ...
    public function update($data)
    {
        ...
        $system_settings = DB::table($this->table)->where('id', $data->id)->first();
        if (empty($system_settings->id)) {
