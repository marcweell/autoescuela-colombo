...
class Survey_person_dataServiceImpl implements ISurvey_person_dataService
{
    private $insertFillables = ['code','survey_person_id','survey_question_option_id','data'];
    private $updateFillables = ['code','survey_person_id','survey_question_option_id','data'];
    private $table =  'survey_person_data';
    private $serviceQuery;

    public function __construct(){
        $this->serviceQuery = new Survey_person_dataServiceQueryImpl();
    }


    public function add($data)
    {
        if (empty($data->survey_person_id)) {
        ...
        $survey_person_data = $this->serviceQuery->findByCode($data->code);

        if (!empty($survey_person_data->id)) {
    ...
    public function update($data)
    {
        ...
        $survey_person_data = $this->serviceQuery->findById($data->id);
        if (empty($survey_person_data->id)) {
