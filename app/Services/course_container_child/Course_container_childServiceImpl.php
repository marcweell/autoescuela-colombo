...
class Course_container_childServiceImpl implements ICourse_container_childService
{
    private $insertFillables = ['name','code'];
    private $updateFillables = ['name','code'];
    private $table =  'course_container_child';
    ...
    public function add($data)
    {
        ...
        $course_container_child = DB::table($this->table)->where('name', $data->name)->first();

        if (!empty($course_container_child->id)) {
            throw new \Exception(__('Nome invalido'), 400);
        }

        
        
        $course_container_child = DB::table($this->table)->where('code', $data->code)->first();
    ...
    public function update($data)
    {
        ...
        $course_container_child = DB::table($this->table)->where('id', $data->id)->first();
        if (empty($course_container_child->id)) {
