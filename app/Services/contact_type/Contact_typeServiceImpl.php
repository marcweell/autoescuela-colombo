...
class Contact_typeServiceImpl implements IContact_typeService
{
    private $insertFillables = ['name','code'];
    private $updateFillables = ['name','code'];
    private $table =  'contact_type';


    public function add($data)
    {
        if (empty($data->name)) {
            throw new \Exception(_('Nome invalido'));
        }
        ...
        $contact_type = DB::table($this->table)->where('name', $data->name)->first();
        ...
        $contact_type = DB::table($this->table)->where('code', $data->code)->first();

        if (!empty($contact_type->id)) {
            throw new \Exception(_('Codigo invalido'));
        }
    ...
    public function update($data)
    {
        ...
        $contact_type = DB::table($this->table)->where('id', $data->id)->first();
        if (empty($contact_type->id)) {
