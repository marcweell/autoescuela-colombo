...
class Document_typeServiceImpl implements IDocument_typeService
{
    ...
    public function add($data)
    {
        ...
        $document_type = DB::table($this->table)->where('name', $data->name)->first();

        if (!empty($document_type->id)) {
            throw new \Exception(__('Nome invalido'), 400);
        }

        
        
        $document_type = DB::table($this->table)->where('code', $data->code)->first();
    ...
    public function update($data)
    {
        ...
        $document_type = DB::table($this->table)->where('id', $data->id)->first();
        if (empty($document_type->id)) {
